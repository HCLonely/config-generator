...
You have to input at least one of them.`),!1}return console.log(r(t)),r(t)}catch(e){return console.error(e),o(e.message),!1}}(e);e&&($("#file-selector,#loadRemoteFileLabel,#loadRemoteFileInput").hide(),e.forEach((a,e)=>{$("div.container").append(`<form id="config-${d(a.name).replace(/[,./;'[\]\\<>?:"{}|`~!@#$%^&*()+=\s]/gi,"")}" style="display:none;" data-type="${a.type||a.filename.split(".").slice(0,-1).join(".")}" data-filename="${a.filename||a.name+"."+a.type}">
        ${a.quote?`<figure class="text-center" style="border: 1px dashed #00c9ff;border-radius: 5px;">
          ...
          <label for="${i}" class="form-label">
            ...
            data-id="${i}" data-name="${n}"
            style="--bs-btn-padding-y: 0rem; --bs-btn-padding-x: .3rem;--bs-btn-font-size: .55rem;border-radius: 50%;margin-left: .5rem;">+</button>`:""}
          </label>
          <textarea type="text" class="form-control item" id="${i}" name="${n}" data-parent="${r}"
            ${o.desp?` aria-describedby="help-${i}"`:""}
            ${o.placeholder?` placeholder="${o.placeholder}"`:""}
            ${o.required?" required":""}
            ${""+o.validation?` value="${o.defaultValue}"`:""}
            ${o.validation?` data-validation="${o.validation}"`:""}
          ></textarea>
          ${o.validation?'<div class="invalid-feedback">Invalid format!</div>':""}
          ${o.desp?`<div id="help-${i}" class="form-text">${o.desp}</div>`:""}
        </div>`):void $("#config-"+r).append(`<div class="mb-3" ${"single-select"===e?` style="display: none;" bind-value="${t}"`:""}>
        <label for="${i}" class="form-label">${o.name||n}${o.required?'<font style="color:red;" title="Required">*</font>':""}
            ${o.validation?'<font style="color:blue;" title="RegExp Validation">!</font>':""}
            ${"array"===e&&!0===o.repeat?`<button type="button"
          class="btn btn-outline-primary repeat" data-id="${i}" data-name="${n}"
          style="--bs-btn-padding-y: 0rem; --bs-btn-padding-x: .3rem;--bs-btn-font-size: .55rem;border-radius: 50%;margin-left: .5rem;">+</button>`:""}
        </label>
        <input type="${o.inputType||"text"}" class="form-control" id="${i}" name="${n}" data-parent="${r}"
          ${o.desp?` aria-describedby="help-${i}"`:""}
          ${o.placeholder?` placeholder="${o.placeholder}"`:""}
          ${o.required?" required":""}
          ${""+o.validation?` value="${o.defaultValue}"`:""}
        ...
        ${o.validation?'<div class="invalid-feedback">Invalid format!</div>':""}
        ${o.desp?`<div id="help-${i}" class="form-text">${o.desp}</div>`:""}
      </div>`);if("boolean"===o.type)return void $("#config-"+r).append(`<div class="form-check form-switch mb-3" ${"single-select"===e?` style="display: none;" bind-value="${t}"`:""}>
        <input class="form-check-input" type="checkbox" role="switch" id="${i}" name="${n}" data-parent="${r}"
          ${o.desp?` aria-describedby="help-${i}"`:""}
          ${o.defaultValue?' checked="checked"':""}
          />
        <label class="form-check-label" for="${i}">${o.name||n}${"array"===e&&!0===o.repeat?`<button type="button"
          class="btn btn-outline-primary repeat" data-id="${i}" data-name="${n}"
          style="--bs-btn-padding-y: 0rem; --bs-btn-padding-x: .3rem;--bs-btn-font-size: .55rem;border-radius: 50%;margin-left: .5rem;">+</button>`:""}
        </label>
        ${o.desp?`<div id="help-${i}" class="form-text">${o.desp}</div>`:""}
      </div>`);if("single-select"===o.type){if($("#config-"+r).append(`<div class="mb-3" ${"single-select"===e?` style="display: none;" bind-value="${t}"`:""}>
        <label class="form-select-label" for="${i}">${o.name||n}${"array"===e&&!0===o.repeat?`<button type="button"
          class="btn btn-outline-primary repeat" data-id="${i}" data-name="${n}"
          style="--bs-btn-padding-y: 0rem; --bs-btn-padding-x: .3rem;--bs-btn-font-size: .55rem;border-radius: 50%;margin-left: .5rem;">+</button>`:""}
        </label>
        <select class="form-select" id="${i}" name="${n}" data-parent="${r}"
        ${o.desp?` aria-describedby="help-${i}"`:""}>
          ${o.options.map((e,t)=>`<option value="${e}" ${e===o.defaultValue?" selected":""}>
          ${o.optionsName?.[t]?o.optionsName[t]:e}</option>`).join("")}
        </select>
        ${o.desp?`<div id="help-${i}" class="form-text">${o.desp}</div>`:""}
      </div>`),o.bindValue){$("#"+i).data("bindData",o.bindValue.body),$("#"+i).change(function(){$(`#config-${r} [bind-value="${n}"]`).remove();const e=$(this).data("bindData")[$(this).val()];e&&Object.entries(e).forEach(([e,t])=>{a(""+r,e,t,o.type,n)})});const l=o.bindValue.body[o.defaultValue];l&&Object.entries(l).forEach(([e,t])=>{a(""+r,e,t,o.type,n)})}return}if("multi-select"===o.type)return void $("#config-"+r).append(`<div class="mb-3" ${"single-select"===e?` style="display: none;" bind-value="${t}"`:""}>
        <label class="form-select-label" for="${i}">${o.name||n}${"array"===e&&!0===o.repeat?`<button type="button"
          class="btn btn-outline-primary repeat" data-id="${i}" data-name="${n}"
          style="--bs-btn-padding-y: 0rem; --bs-btn-padding-x: .3rem;--bs-btn-font-size: .55rem;border-radius: 50%;margin-left: .5rem;">+</button>`:""}
        </label>
        <select class="form-select" id="${i}" name="${n}" multiple data-parent="${r}"
        ${o.desp?` aria-describedby="help-${i}"`:""}>
      ...
      </div>`),Object.entries(o.body).forEach(([e,t])=>{a(r+"-"+n,e,t,o.type)}));if("array"===o.type){$("#config-"+r).append(`<div class="card card-body" style="padding-bottom:0;margin-bottom:1rem;${"single-select"===e?"display: none;":""}"
      ${"single-select"===e?` bind-value="${t}"`:""}>
        <p style="text-align:center;margin-bottom:0;">
          <a class="btn btn-primary" data-bs-toggle="collapse" href="#config-${r}-${n}" role="button" aria-expanded="true"
            aria-controls="config-${r}-${n}" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-custom-class="custom-tooltip"
