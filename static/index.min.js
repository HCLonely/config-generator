(async()=>{var e=Object.fromEntries(window.location.search.replace(/^\?/,"").split("&").map(e=>e.split("=")))["fileLink"],[t]=(e&&((t=new URL(decodeURIComponent(e))).search?t.search+="&time="+Date.now():t.search+="?time="+Date.now(),a(decodeURIComponent(e))),$("#file-selector"));async function a(e){var[e,t]=await axios.get(e,{validateStatus:e=>200<=e&&e<400}).then(e=>[!0,e.data]).catch(e=>(console.error(e),[!1,e]));e?r(t):i(t.message,"Get template file failed!")}function r(e){var e=function(e){try{var t=jsyaml.load(e);if(console.log(t),!Array.isArray(t))return i("The root template must be an array!"),!1;for(let e=0;e<t.length;e++){if(!t[e].name)return i(`The template of No.${e+1} has no the key: "name".`),!1;if(!t[e].body)return i(`The template of No.${e+1} has no the key: "body".`),!1;if(!t[e].type&&!t[e].filename)return i(`The template of No.${e+1} has no the keys: "type", "filename".
You have to input at least one of them.`),!1}return t}catch(e){return console.error(e),i(e.message),!1}}(e);e&&($("#file-selector,#loadRemoteFileLabel,#loadRemoteFileInput").hide(),e.forEach((a,e)=>{$("div.container").append(`<form id="config-${a.name}" style="display:none;" data-type="${a.type||a.filename.split(".").slice(0,-1).join(".")}" data-filename="${a.filename||a.name+"."+a.type}"></form>`),0===e&&($("#single-config-name>button").attr("data-name",a.name),$("#single-config-name>button").text(a.name),$("#config-"+a.name).show());e=$(`<li><a class="dropdown-item${0===e?" active":""}" href="javascript:void(0);">${a.name}</a></li>`);e.click(function(){var e=$(this).text().trim();$("#single-config-name>button").attr("data-name",e),$("#single-config-name>button").text(e),$("#single-config-name li>a").removeClass("active"),$(this).children("a").addClass("active"),$("form").hide(),$("#config-"+e).show()}),$("#single-config-name>ul").append(e),Object.entries(a.body).forEach(([e,t])=>{!function a(r,n,i,e){const t=r+"-"+n;if("text"===i.type)return"textarea"===i.inputType?void $("#config-"+r).append(`<div class="mb-3">
